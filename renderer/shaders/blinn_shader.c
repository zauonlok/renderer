...
static vec4_t calculate_diffuse(vec2_t uv, blinn_uniforms_t *uniforms) {
...
static vec4_t calculate_emission(vec2_t uv, blinn_uniforms_t *uniforms) {
...
static vec4_t calculate_specular(vec2_t uv, blinn_uniforms_t *uniforms) {
...
static float calculate_specular_strength(vec3_t light_dir, vec3_t eye_dir,
    ...
    vec3_t halfway = vec3_normalize(vec3_add(light_dir, eye_dir));
    float base = max_float(-vec3_dot(halfway, normal), 0);
